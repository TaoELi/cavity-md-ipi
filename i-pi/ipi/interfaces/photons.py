...
class photons:
    def __init__(self):
        ...
        if self.apply_photon:
            ...
            # only preserve the odd quantum number, because others are decoupled [1, 3, 5, 7, ...] * omega
            self.freq_lst = self.freq * np.array([n for n in range(1, self.nmodes+1)]*2)
            # the corresponding phase should be 1, -1, 1, -1, etc
            self.E0_lst = self.E0 * np.ones(self.nmodes*2) * self.freq_lst / self.freq
            #self.E0_lst = np.array( [x * np.real(1j**(i*2)) for i, x in enumerate(self.E0_lst[0:self.nmodes])]*2 )
            # The phase should be
