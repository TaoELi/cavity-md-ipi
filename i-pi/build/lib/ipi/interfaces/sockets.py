...
class Driver(DriverSocket):
    ...
    def __init__(self, socket):
        ...
        self.photon = photon()
        self.photon2 = photon()
        self.apply_photon = self.photon.apply_photon
        # End Tao's modification
    ...
    def sendpos(self, pos, h_ih):
        ...
        self.pos_no_photon = np.array(pos)
        if self.apply_photon:
            self.pos_no_photon = self.pos_no_photon[:-6]
            self.photon.update_pos(np.array(self.pos[-6:-3]))
            self.photon2.update_pos(np.array(self.pos[-3:]))
            self.dipole.update_pos(self.pos_no_photon)
        ...
        if (self.status & Status.Ready):
            try:
                ...
                #self.sendall(pos)
                nphoton = 2 if self.apply_photon else 0
    ...
    def getforce(self):
        ...
        if self.apply_photon:
            # 1. Modify energy
            mu_photon = self.photon.obtain_potential() + self.photon2.obtain_potential()
            Ex = self.photon.obtain_Ex()
            Ey = self.photon2.obtain_Ey()

            dipole_x_tot = self.dipole.calc_dipoles_x_tot()
            dipole_y_tot = self.dipole.calc_dipoles_y_tot()
            #dipole_z_tot = self.dipole.calc_dipoles_z_tot()
            #print "dipole x which = %.3E\t %.3E\t %.3E" %(dipole_x_tot, dipole_y_tot, dipole_z_tot)

            mu_int = Ex * dipole_x_tot + Ey * dipole_y_tot
            mu += mu_photon + mu_int + 0.5 * self.photon.coeff_self * (dipole_x_tot**2 + dipole_y_tot**2)
            # 2. Modify force
            mf[0::3] += - (Ex + self.photon.coeff_self * dipole_x_tot)  * self.dipole.calc_dmudx()
            mf[1::3] += - (Ey + self.photon.coeff_self * dipole_y_tot)  * self.dipole.calc_dmudy()
            #if self.photon.step > self.photon.step_on:
            #    mf[0::3] -= (2.0 * self.photon.pot_coeff * (self.photon.pos[0] + self.photon.E0 * dipole_x_tot) * self.photon.E0) * self.dipole.calc_dmudx()
                #mf[1::3] -= (2.0 * self.photon.pot_coeff * (self.photon2.pos[1] + self.photon.E0 * dipole_y_tot) * self.photon.E0) * self.dipole.calc_dmudy()
            ...
            #if self.photon.step > self.photon.step_on:
            #    f_photon[0] -=  2.0 * self.photon.pot_coeff * (self.photon.E0 * dipole_x_tot)
                #f_photon[4] -=  2.0 * self.photon.pot_coeff * (self.photon.E0 * dipole_y_tot)
