...
class Driver(DriverSocket):
    ...
    def sendpos(self, pos, h_ih):
        ...
        In sending position data, I need to send only N-1 atoms' positions.
        """
        self.pos_no_photon = pos.copy()
        if self.apply_photon:
            self.pos_no_photon = self.pos_no_photon[:-3]
            self.photon.update_pos(pos[-3:])
        ...
        if (self.status & Status.Ready):
            try:
                ...
                #self.sendall(pos)
                nphoton = 1 if self.apply_photon else 0
    ...
    def getforce(self):
        ...
        if self.apply_photon:
            # 1. Modify energy
            mu_photon = self.photon.obtain_potential()
            Ex = self.photon.obtain_Ex()
            dipole_x_tot = np.sum(self.dipole.calc_dipoles_x(self.pos_no_photon))
            mu_int = Ex * dipole_x_tot
            mu += mu_photon + mu_int
            # 2. Modify force
            mf[0::3] += - Ex * self.dipole.calc_dmudx(self.pos_no_photon)
            f_photon = np.zeros(3, np.float64)
            f_photon = self.photon.obtain_bare_force()
