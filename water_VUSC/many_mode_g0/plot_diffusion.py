...
def plot_diffusion():
    ...
    colors = ["k", "c--", "r-."]
    labels = ["outside cavity", "in cavity $\widetilde{\\varepsilon}=4\\times 10^{-4}$", "in cavity $\widetilde{\\varepsilon}=6\\times 10^{-4}$"]
    axes = clp.initialize(2, 1, width=4.3, height=5, LaTeX=True, fontsize=12,
            labelthem=True, labelthemPosition=[0.98, 0.92])
    ts, corrs, freqs, sps = [], [], [], []
    patterns = ["simu_*.x_*.xyz.vac.txt", "simu_*.xc.xyz.vac.txt"]
    for i, path in enumerate(paths):
        data = obtain_avg_data(path, pattern=patterns[1])
    ...
    clp.adjust(savefile="Diffusion_multimode.pdf", tight_layout=True)
