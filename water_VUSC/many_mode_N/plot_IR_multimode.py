...
def plot_IR():
    Ns = [1, 4, 6, 10]
    paths = ["N_1", "N_4", "N_6", "N_10"]
    colors = ["k", "k", "k", "k"]
    labels = ["one mode", "four modes", "six modes", "ten modes"]
    ...
    for i, path in enumerate(paths):
        ...
        if Ns[i] is 1:
            axes[i].axvline(x=3550, alpha=0.5)
        else:
            for j in range(Ns[i]):
                axes[i].axvline(x=3550.0/Ns[i]*2.0*(j+1), alpha=0.5)
        axes[i].legend(loc="upper right")
    clp.adjust(savefile="IR_multimode.pdf")
