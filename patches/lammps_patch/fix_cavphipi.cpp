...
FixCAVPHIPI::FixCAVPHIPI(LAMMPS *lmp, int narg, char **arg) :
  Fix(lmp, narg, arg), irregular(NULL)
{
  /* format for fix:
   *  fix  num  group_id cavphipi host port [unix]
   */
  if (strcmp(style,"cavphipi") != 0 && narg < 5)
    error->all(FLERR,"Illegal fix cavphipi command");

  if (atom->tag_enable == 0)
    error->all(FLERR,"Cannot use fix cavphipi without atom IDs");

  if (atom->tag_consecutive() == 0)
    error->all(FLERR,"Fix cavphipi requires consecutive atom IDs");

  if (strcmp(arg[1],"all"))
    error->warning(FLERR,"Fix cavphipi always uses group all");
...
FixCAVPHIPI::~FixCAVPHIPI()
{
  if (bsize) delete[] buffer;
  if (bsize) delete[] buffer_q;
  free(host);
...
void FixCAVPHIPI::initial_integrate(int /*vflag*/)
{
  ...
  MPI_Bcast(buffer_q,nat,MPI_DOUBLE,0,world);
...
void FixCAVPHIPI::final_integrate()
{
  ...
  // 2021-07-09 Tao E. Li's main modification
  // Obtain the charge of atoms at every time step, which is used espeically for ReaxFF forcefield
  double *q = atom->q;
  double *lbuf_q = new double[nat];
  // reassembles the charge vector from the local arrays
  for (int i = 0; i < nat; ++i) lbuf_q[i]=0.0;
  for (int i = 0; i < nlocal; i++) {
    lbuf_q[atom->tag[i]-1]=q[i];
  }
  MPI_Allreduce(lbuf_q,buffer_q,nat,MPI_DOUBLE,MPI_SUM,world);
  delete[] lbuf_q;
  // END of 2021-07-09 modification
  ...
  if (master) {
    ...
    if (strcmp(header,"GETFORCE    ") == 0 )  {
      ...
      writebuffer(ipisock,(char*) buffer_q, nat*8, error);
