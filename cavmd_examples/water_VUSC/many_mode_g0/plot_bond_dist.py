...
def plot_bond_distribution():
    ...
    colors = ["k", "c--", "r-."]
    labels = ["outside cavity", "in cavity $\widetilde{\\varepsilon}=4\\times 10^{-4}$", "in cavity $\widetilde{\\varepsilon}=6\\times 10^{-4}$", "8"]
    xs, ys = [], []
    for path in paths:
        data = obtain_avg_data(path, pattern="simu_*.xc.xyz.bond_length_dist.txt")
        x, y = data[:,0], data[:,1]
        y /= np.sum(y) * (x[2] - x[1])
        xs.append(x)
        ys.append(y)
    ...
    clp.adjust(tight_layout=True, savefile="bond_dist_multimode.pdf")
